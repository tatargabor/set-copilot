...
- **`src/audio.ts`** — spawns `parec` (Linux) / `sox` (macOS) and hands back raw 16 kHz s16le mono streams. Prefers `/usr/bin/parec` over a PATH-shadowing Homebrew build, which hangs silently against PipeWire.
  ...
  **Every macOS audio failure looks the same from the outside: the capture runs, errors nothing, and streams zero bytes.** Three separate causes were found behind that one symptom on 2026-08-05, so don't read a 0-byte capture as a single fault:
  - **A device name from another machine.** `micSource` names *hardware*, so it belongs in the **user-level** config, never the committed project one — a Linux `alsa_input.usb-…` name means nothing to CoreAudio, and sox opens *something*, prints no error, and delivers nothing. Three things enforce that: `audio` merges per key like the other nested sections (it was the one left out, so a project declaring `sampleRate` silently discarded the machine's mic), `diagnoseConfig` warns on a device pinned in the project config, and the env vars resolve with `??` so `MIC_SOURCE=` (set but empty) can say "no device" — `loadDotEnvFile` tests `=== undefined` for the same reason. With `||` that escape hatch fell through to the value it was set to escape.
  - **The Microphone permission belongs to the enclosing `.app`**, not to sox and not to node — a shell inside an unticked Terminal/iTerm/Zed records silence, and the grant only reaches processes started *after* it. `doctor` walks the process ancestry (`hostAppFromAncestry`) and names that app, because "wrong device or broken sox" sends the reader to edit a config that was never the problem.
  - **`listSources()` used to lie.** It asked `sox --help-device coreaudio`; current Homebrew sox rejects the flag, and its usage error came back *as the device list*. It now reads `system_profiler SPAudioDataType -json` (`parseMacInputDevices`, pure and unit-tested against a real payload) — inputs only, default first, bare name at the start of the line so it is copy-pasteable into `micSource`.
