...
- [x] 1.1 `server.ts`: keep a fixed-size scroll-history ring buffer per `scroll` category (N config,
  default 20 via `src/config.ts`)
- [x] 1.2 Extend `replay()` to send the last N scroll lines per scroll category, zone-filtered like
  the live stream
- [x] 1.3 Extend `accumulate()` to feed the ring, so the startup drain over `wall-events.jsonl`
  rebuilds the scroll-history too (reusing the existing shared-offset once-only guarantee)
- [x] 1.4 Unit tests (pure logic): ring eviction at N, rebuild determinism (same log → same ring),
  no double-count across rebuild+tail, zone-filtering of replayed scroll lines

## 2. Safe log reset

- [x] 2.1 `wall --reset`: rotate `wall-events.jsonl` aside (timestamped rename), never truncate a
  live one — mirror the transcript hand-over invariant
- [x] 2.2 Document the operational rule: during live use do not truncate the log or restart
