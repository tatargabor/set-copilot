...
- [x] 8.2 Record a second run of the same scenario. What this measured turned out to matter more than interactive-vs-headless: **the measure's own noise**. Two runs, identical code and conditions, moved coverage 0.857 → 0.571. A single-run comparison would have reported "regressed" with nothing changed. [REQ: Two runs of one scenario are comparable]
- [x] 8.5 Separate the noise's two sources. Re-judging ONE unchanged run three times gave identical coverage and an identical missed set — the judge is stable; the copilot is the variable one. Latency did move (36.0s → 40.3s) by matching a later restatement, which the judge prompt now forbids: three re-judgements are now byte-identical. [REQ: Mechanical and judged dimensions are separated]
- [x] 8.6 Make the measure able to detect a real change, given that noise. Seven planted moments means one moment is worth 14 coverage points; N runs and a median, or a scenario with more moments, or both. Until then a single-run comparison is not evidence. [REQ: Two runs of one scenario are comparable]
...
- [x] AC-1: A live copilot session polling a replayed runtime dir behaves as against a live capture, with no configuration or code difference. [REQ: A replay is indistinguishable from a live capture to its consumers, scenario: A polling consumer receives replayed lines as ordinary transcript lines]
- [x] AC-2: While a replay runs the capture reads as alive; when it ends the consumer observes a normal capture end. [REQ: A replay is indistinguishable from a live capture to its consumers, scenario: The end of the scenario ends the capture]
- [x] AC-3: At real time, the gap between two played lines matches the gap between their timestamps, and two-channel interleaving is preserved. [REQ: Playback is paced by the scenario's own timestamps, scenario: Two channels interleave as recorded]
- [x] AC-4: A replay against a runtime dir with a live capture refuses to start and leaves the transcript untouched. [REQ: A replay never collides with a live capture, scenario: A live capture blocks a replay]
- [x] AC-5: An unconsumed transcript in the target runtime dir is archived, never truncated. [REQ: A replay never collides with a live capture, scenario: An unconsumed transcript is not silently overwritten]
- [x] AC-6: A scenario whose expectations contain no planted moments is rejected. [REQ: A scenario plants ground truth deliberately, scenario: A scenario without planted moments is rejected]
- [x] AC-7: An operator can follow an entire scenario — time, section, speaker, text, planted moments — from the timeline alone. [REQ: The timeline is readable and matches the lines, scenario: An operator can read what will be played]
- [x] AC-8: Editing a scenario's lines without regenerating its timeline is reported as stale. [REQ: The timeline is regenerated from the lines, scenario: The timeline is regenerated from the lines]
- [x] AC-9: A scenario authored from unrelated source material plays and scores with no harness change. [REQ: A scenario is authored from any source material, scenario: A second scenario needs no code change]
- [x] AC-10: A planted moment that draws no reaction is reported as a miss naming what was expected. [REQ: The scorecard measures reaction, drawing, judgement, and noise, scenario: A missed planted moment is counted]
- [x] AC-11: A reaction matching no planted moment counts against precision rather than being ignored. [REQ: The scorecard measures reaction, drawing, judgement, and noise, scenario: A reaction with no planted moment behind it is counted separately]
- [x] AC-12: A scorecard from a sped-up run reports its latency dimensions as invalid, and its content dimensions normally. [REQ: A run may only report figures its playback speed supports, scenario: A sped-up run reports no latency]
- [x] AC-13: Comparing two scorecards of the same scenario version reports each dimension as improved, regressed, or unchanged. [REQ: Two runs of one scenario are comparable, scenario: A regression is visible]
- [x] AC-14: Comparing scorecards across different scenario fingerprints refuses a verdict and says the measuring stick changed. [REQ: Two runs of one scenario are comparable, scenario: A scenario change invalidates the comparison]
- [x] AC-15: A dimension whose evidence is missing is reported as unmeasured with a reason, and the rest still score. [REQ: A score is computed from the run's own artifacts, scenario: Missing evidence is reported, not guessed]
