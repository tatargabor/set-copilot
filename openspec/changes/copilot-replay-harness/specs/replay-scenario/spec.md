...
- The scenario's three parts and how they relate.
- What the planted expectations must record for a run to be scorable.
- The readable timeline and the requirement that it stay truthful to the lines.
- Source-material independence.

## OUT OF SCOPE

- Playing a scenario (see `transcript-replay`).
- Computing a score (see `replay-scorecard`).
- Any claim that a generated scenario is a faithful prediction of what a real presenter
  would say — it is a fixture, and its value is that it is constant.
