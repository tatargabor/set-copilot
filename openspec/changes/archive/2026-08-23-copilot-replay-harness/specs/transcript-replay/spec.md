...
- Pacing a scenario's lines into the transcript file by their own timestamps.
- Speed control, and the validity consequences of using it.
- Runtime-dir ownership: the PID file, refusal against a live capture, end-of-run.
- Reporting where the run currently stands.

## OUT OF SCOPE

- Authoring or generating scenarios (see `replay-scenario`).
- Scoring what the copilot did (see `replay-scorecard`).
- Any modification of `poll`, the wall, the mirror, or the shipped skills — the fact
  that they need none is the property this capability relies on.
