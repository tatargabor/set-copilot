...
- Which evidence a score is computed from.
- The measured dimensions and what each one means.
- The validity rules that decide which figures a given run may report.
- Comparing two runs of the same scenario.

## OUT OF SCOPE

- Deciding whether a score is good enough to ship — that is an operator judgement.
- Scoring anything but a replay run against its own scenario. A live meeting has no
  ground truth and cannot be scored this way.
