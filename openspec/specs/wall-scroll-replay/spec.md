...
Make a reloading or late-joining wall window recover its recent scroll text on connect — not just
graphs and the pinned `latest` — and give a fresh run a safe way to reset the event log without
destroying the accumulated state's rebuild source.
