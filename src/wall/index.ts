...
export async function runWall(cfg: CopilotConfig, opts: RunWallOptions = {}): Promise<WallServer> {
  ...
  for (let attempt = 0; attempt < PORT_FALLBACK_TRIES; attempt++) {
    ...
    const candidate = new WallServer({ port, windows, registry, publicDir, projectRoot: cfg.projectRoot, redaction: cfg.wall.redaction, scrollHistory: cfg.wall.scrollHistory, runtimeDir: cfg.runtimeDir, transcriptPath: cfg.transcriptOutput });
